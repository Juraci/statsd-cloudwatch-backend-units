...
var Backend = module.exports = function(options, time, binder, logger, units) {
  ...
  console.log('Units received: ' + this.units)
...
_.extend(Backend.prototype, {
  flush: function(time, metrics) {
    ...
    var data = _.union(
      ...
      collect_gauges(date, metrics.gauges, this.dimensions, this.units))
...
function collect_gauges(date, gauges, dimensions, units) {
    console.log('collect_gauges')
  var metrics = []
  for (var key in filter_metrics(gauges)) {
    var value = gauges[key]
    metrics.push({ MetricName: key, Value: value,
        Unit: (function(units_wtf, metric) {
            for(key in units_wtf) {
                if(metric.includes(key)) {
                    console.log('key -> ' + key)
                    console.log('metric -> ' + metric)
                    return units_wtf[key] + '';
                }
            }
            return 'None';
        })(units, key),
